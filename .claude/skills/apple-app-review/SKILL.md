...
If the repo (or your studio) has deterministic release tooling — metadata validators, privacy/telemetry audit scripts, launch-packet checks — prefer it over duplicate source scans. In this repo that includes:

```sh
python3 Marketing/AppStoreMetadata/validate_metadata.py <metadata-json>   # field limits per docs/schemas/launch-packet.v1.schema.json
```

plus any repo-local audit scripts. Run only documented commands. Preserve their raw findings and identify the tool version/commit when possible.
...
Add a `Tooling follow-ups` section. For any issue your deterministic tooling could have detected mechanically but did not, propose:
...
Do not silently patch the tooling during an app audit. A policy judgment or runtime-only behavior belongs in the manual checklist, not a brittle grep rule.
