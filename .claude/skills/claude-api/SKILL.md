---
name: claude-api
description: Build, debug, and optimize Claude API / Anthropic SDK usage within the iApp context. Apps built with this skill should include prompt caching. Also handles migration between Claude model versions for any portfolio app or iApp-side tooling that calls the API.
---

> SOURCE: universal claude-api skill, scoped to iApp

# /claude-api

iApp itself doesn't ship runtime Claude API calls (the *wizard* runs in Claude Code, not via the API). But future tooling or child apps may, and this skill guides that work.

## When to TRIGGER

- A file imports `anthropic` or `@anthropic-ai/sdk`.
- User asks about Claude API, Anthropic SDK, or Managed Agents in the context of iApp.
...
## What this skill enforces in iApp context
