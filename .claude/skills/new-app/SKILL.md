...
- Building a one-off outside your portfolio conventions — Kindling is opinionated for a coherent portfolio; a throwaway prototype doesn't need the full wizard.
...
0. **Spec / JTBD intake** *(front door — new in v2)* — `AskUserQuestion`-driven, six fields per [DECISIONS/TEMPLATE-spec.md](../../../DECISIONS/TEMPLATE-spec.md):
   ...
   **Mockup intake (part of step 0):** ask whether the user has existing designs — Figma/Sketch/Canva exports, prototype stills, or photographed paper sketches. If yes, have them export per [docs/BRING_YOUR_MOCKUPS.md](../../../docs/BRING_YOUR_MOCKUPS.md) into `drafts/<app-name>/mockups/` (numbered in first-run order), then **view every image** before proceeding. If a design-tool MCP server is connected to the session (e.g. Figma Dev Mode), prefer it: enumerate all screens in the file, read exact color/typography values for the design steps, and consult it again during Phase B screen-building — but still have the user export a few PNGs into `mockups/` as the draft's permanent design record. Mockups become evidence for the design steps: the file ordering seeds step 11's first-sixty-seconds narration, the screens list feeds `016-spec.md`, and steps 6/7/12 (visual identity, palette, typography) read them per their own skills. Mockups inform, taste rules still gate — surface any rule-forced delta (AAA contrast nudges, `#000`/`#FFF` offsets) to the user explicitly; never silently "correct" their design. Ask how final the mockups are (directional vs pixel-final) and read them accordingly.
...
5. **Push to the user's repo** — if the sibling directory already has an `origin` remote (the pre-created blank private repo from [docs/YOUR_OWN_REPO.md](../../../docs/YOUR_OWN_REPO.md)), commit the scaffold on top of its README history on a `claude/initial-scaffold-XXXXX` branch and push there — do NOT create a new repo. Only when no remote exists: `mcp__github__create_repository` (visibility chosen via `AskUserQuestion` — default **private**) → `mcp__github__create_branch` → `mcp__github__push_files`.
6. **Run validation** — `/validate-template` skill against the new repo (build green, all 12 housekeeping/feature tests pass).
7. **Open back-PR to Kindling** — update `PORTFOLIO.md` (new app row), `PALETTE_CATALOG.md` (claim seed), `MONETIZATION_MATRIX.md` (new row), `RECENT_LEARNINGS.md` if anything novel surfaced.
