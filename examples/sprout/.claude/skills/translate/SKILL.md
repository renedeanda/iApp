...
Fill missing translations in `Sprout/Localizable.xcstrings` (and `InfoPlist.xcstrings`) for one locale.
...
1. Read `Sprout/Localizable.xcstrings` (JSON). For each key, find entries missing the target locale, or whose target localization has `state: "new"` / is absent.
...
6. Run `xcodebuild test -only-testing:SproutTests/LocalizationParityTests` to confirm parity.
