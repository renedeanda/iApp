---
name: generate-icons
description: Regenerate the AppIcon.appiconset (plus splash, notification, and store icons) from the SVG source at Sprout/Resources/icon_master.svg. Use when the user wants to update, regenerate, or modify app icons.
---
...
- After editing `Sprout/Resources/icon_master.svg`.
...
- `Sprout/Assets.xcassets/AppIcon.appiconset/` — every required size: 1024 (App Store), plus the device sizes (60@2x/3x, 76@2x, 83.5@2x, 167, 152, 40, 29, 20 families). The `Contents.json` references them all.
- `Sprout/Assets.xcassets/LaunchBackground.colorset/` stays palette-driven (not regenerated here).
...
1. Verify `Sprout/Resources/icon_master.svg` exists and is valid (`xmllint --noout` if available; otherwise trust qlmanage to surface a render failure).
2. Invoke `bin/generate-icons.sh`:
   ```sh
   bin/generate-icons.sh \
     Sprout/Resources/icon_master.svg \
     Sprout/Assets.xcassets/AppIcon.appiconset
   ```
...
3. Run `xcodebuild test -only-testing:SproutTests/AppIconAssetTests` — confirms the full set is present and non-zero.
...
Source: Sprout/Resources/icon_master.svg
Renderer: qlmanage + sips (macOS-builtin)

✓ Sprout/Assets.xcassets/AppIcon.appiconset — 7 icon variants rendered
