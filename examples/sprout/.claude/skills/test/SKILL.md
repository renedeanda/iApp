...
Run the test suite. Default: everything. With arg: `-only-testing:SproutTests/<SuiteName>`.
...
2. Run:
   ...
   ```
   (or `-only-testing:SproutTests/<Suite>` to filter.)
...
```
Test Suite 'SproutTests' — 8 housekeeping + N feature tests
  ✓ ThemeContrastTests          (4 tests)
  ✓ LocalizationParityTests     (7 tests)
  ✗ MyFeatureTests.handlesEmpty — expected ... got ... at Sprout/Feature.swift:88
