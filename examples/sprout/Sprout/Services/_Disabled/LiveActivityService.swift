...
// see Widgets/SproutLiveActivity.swift and recipes/swift/add-live-activity.md.
...
/// = YES` to the host Info.plist, and uncomment `SproutLiveActivity()`
/// in Widgets/SproutWidgetBundle.swift.
///
/// `SproutActivityAttributes` is defined in
/// SproutWidgets/SproutActivityAttributes.swift — shared between the app
/// target (this file) and the widget extension target (which renders
/// it in SproutLiveActivity.swift).
@MainActor
final class LiveActivityService {
    static let shared = LiveActivityService()

    private var activity: Activity<SproutActivityAttributes>?

    /// Begin an activity. Throws if the user disabled Live Activities
    /// for the app — handle it, don't force-try.
    func start(title: String, initialState: SproutActivityAttributes.ContentState) throws {
        guard ActivityAuthorizationInfo().areActivitiesEnabled else { return }
        let attributes = SproutActivityAttributes(title: title)
    ...
    func update(_ state: SproutActivityAttributes.ContentState) {
    ...
    func end(final state: SproutActivityAttributes.ContentState) {
        guard let activity else { return }
        Task {
