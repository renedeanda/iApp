...
struct SproutWidgetEntry: TimelineEntry {
    let date: Date
    let payload: WidgetPayload
}

struct SproutWidgetProvider: TimelineProvider {
    func placeholder(in context: Context) -> SproutWidgetEntry {
        SproutWidgetEntry(date: .now, payload: .placeholder)
    }

    func getSnapshot(in context: Context, completion: @escaping (SproutWidgetEntry) -> Void) {
        completion(SproutWidgetEntry(date: .now, payload: WidgetSharedData.read()))
    }

    func getTimeline(in context: Context, completion: @escaping (Timeline<SproutWidgetEntry>) -> Void) {
        // Triggered strategy: one entry, valid until the app reloads us.
        let entry = SproutWidgetEntry(date: .now, payload: WidgetSharedData.read())
...
struct SproutWidget: Widget {
    ...
    var body: some WidgetConfiguration {
        StaticConfiguration(kind: kind, provider: SproutWidgetProvider()) { entry in
