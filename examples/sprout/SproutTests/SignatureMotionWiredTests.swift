...
struct SignatureMotionWiredTests {
    ...
    @Test func signatureMotionFileExists() {
        let modifier = Self.repoRoot
            .appendingPathComponent("Sprout/Theme/SignatureMotion.swift")
        #expect(
    ...
    @Test func signatureMotionIsApplied() {
        let views = Self.repoRoot.appendingPathComponent("Sprout/Views")
        ...
        #expect(
            applied,
            "No .signatureMotion() call site under Sprout/Views — DECISIONS/009 produced an ADR but no wired code (see RECENT_LEARNINGS: an ADR that produces no code is a gap)"
        )
    }
    ...
    /// levels up is the repo. (`rename-template.sh` rewrites `Sprout`.)
