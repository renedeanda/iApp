...
> Repo-local context for release agents and tooling.
> The wizard substitutes Seed tokens at `/new-app --commit`; refresh this doc
> after major changes.
...
- In-app locales: en, es, de, fr, pt, ja, zh-Hans (tier-1; see `i18n/`)
...
- Native iOS widgets (if enabled) load strings from the widget target's own bundle —
  keep widget localization in parity with the app.
...
- No current blockers or warnings — re-audit before each submission.
