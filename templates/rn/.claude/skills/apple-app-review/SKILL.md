...
If the project ships deterministic release validators (metadata validation, privacy audits, telemetry audits), prefer them over duplicate source scans. Run only their documented commands, preserve their raw findings, and identify the tool version/commit when possible.
...
### 8. Feed deterministic gaps into release tooling

Add a `Tooling follow-ups` section. For any issue a deterministic validator could have detected mechanically but did not, propose:
...
Do not silently patch release tooling during an app audit. A policy judgment or runtime-only behavior belongs in the manual checklist, not a brittle grep rule.
